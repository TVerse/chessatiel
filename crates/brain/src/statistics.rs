...
impl Display for Statistics {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        ...
        writeln!(f, "nodes searched at current depth: {}", self.nodes_searched_this_depth)?;
        write!(f, "nodes searched total: {}", self.nodes_searched)?;
        Ok(())
    }
}
...
impl StatisticsHolder {
    pub fn new() -> Self {
        Self { stats: StatisticsInternal::default() }
    }

    pub fn node_searched(&self) {
        self.stats.nodes_searched_this_depth.fetch_add(1, Ordering::Relaxed);
        self.stats.nodes_searched.fetch_add(1, Ordering::Relaxed);
    }

    pub fn depth_changed(&self, new_depth: u64) {
        self.stats.nodes_searched_this_depth.store(0, Ordering::Relaxed);
    ...
    pub fn get_statistics(&self) -> Statistics {
        ...
        let nodes_searched_this_depth = self.stats.nodes_searched_this_depth.load(Ordering::Relaxed);
